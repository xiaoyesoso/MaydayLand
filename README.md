...
MaydayLand 用 **"歌词情绪标签 + LBS 城市角落 + 同好暗号 + 人格测评"** 四位一体的产品形态，回应五迷四大未被满足的真实需求：
...
- 🧠 **personality-quiz**（v1.2）：五月天全曲库人格测评，20 题 5 维度，生成复合人格标题、人生代表曲、共鸣 Top3、匹配 Top5 与 Canvas 测评海报
...
**服务保活**：生产环境启动后，`app/keepalive.py` 会每 20 分钟访问一次云托管域名，防止 30 分钟无访问被平台回收。
...
- **核心组件**：LyricBanner（启动页动画）/ CornerCard / ConcertCard / PasscodeCard / MaydayBalls / QuizPoster
...
| 评审维度 | 权重 | 关键证据 |
| :--- | :---: | :--- |
| **创新性** | 30% | 三大未被满足需求 + "歌词情绪标签 × LBS × 同好暗号 × 人格测评"差异化形态 |
| **实用性** | 30% | 18-35 岁五迷真实场景（演唱会续场 / 周末 citywalk / 出差打卡）；全链路可点击；2026 最新真实数据 |
| **完成度** | 20% | 发现 → 详情 → 打卡 / 对暗号 → 分享 → 测评 → 我的 全链路可点击；11 capabilities + Canvas 海报 + 徽章墙 |
